      ...
      <FadeInScroll>
        ...
        {Object.entries(CHARACTER_DESIGN_BLOCKS).map(([projectName, projectData], i) => (
          <div key={i} className="mb-52 mt-15">
            <h2 className="text-xl md:text-3xl font-bold mb-8 text-black font-magilio text-center">{projectData.title}</h2>
            {projectData.blocks.map((block, j) => (
              <div key={j}>
                {block.title && <h1 className="text-lg md:text-2xl text-black text-center font-magilio mt-20 mb-10">{block.title}</h1>}
